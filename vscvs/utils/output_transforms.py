...
def output_transform_multimodal_gan_trainer(_embeddings, _mode_predictions, _mode_labels, _generator_labels, _classes,
    ...
    :type: torch.nn.Module
    :return: value to be assigned to engine's `state.output` after each iteration, which in a GAN formulation is a
...
def output_transform_siamese_evaluator(embeddings_0, embeddings_1, target):
    ...
    :type: torch.Tensor
    :return: the value to be assigned to engine's `state.output` after each iteration, which must fit that expected by
    the metrics. By default, in a siamese network, it is the embeddings of each image pair and their target tensor.
